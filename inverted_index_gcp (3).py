    ...
    def read(self, locs, n_bytes, directory):
        ...
        for f_name, offset in locs:
            if f_name not in self._open_files:
                self._open_files[f_name] = open(f'/home/hodayat/{directory}/postings_gcp/{f_name}', 'rb')
...
class InvertedIndex:
    ...
    @staticmethod
    def read_posting_list(index, w, directory):
        with closing(MultiFileReader()) as reader:
            locs = index.posting_locs[w]
            b = reader.read(locs, index.df[w] * TUPLE_SIZE, directory)
            term_total = 0
            posting_list = []
